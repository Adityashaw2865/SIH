...
// FIX: this returns the FULL patient record (all clinical answers,
// documents, PII) via serializePatient — it MUST require staff auth,
// otherwise anyone who knows/guesses a patient's Mongo _id could read
// their entire medical record with no login at all.
patientsRouter.get("/:id", requireAuth, requireRole("triage", "doctor", "admin"), async (req, res, next) => {
  try {
